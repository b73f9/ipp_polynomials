/** @file
   Nagłówek pozwalający na przekierowanie funkcji do testów
...
/* Redirect read to a function in the test application so it's possible to
 * test the standard input. */
#ifdef read
#undef read
#endif /* read */
#define read(fd, buf, count) mock_read((fd), (buf), (count))
extern int mock_read(int fd, void *buf, size_t count);
 ...
 * function here. */
#define main(...) calc_main(__VA_ARGS__)
